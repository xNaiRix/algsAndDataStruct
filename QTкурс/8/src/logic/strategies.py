...
class SaveStrategy(ABC):
    @abstractmethod
    def save(self, filename: str, scene:"QGraphicsScene")->None:
        pass
...
class JsonSaveStrategy(SaveStrategy):
    def save(self, filename:str, scene:"QGraphicsScene")->None:
        ...
        items = scene.items()[::-1]
        for item in items:
            if hasattr(item, "in_group") and item.in_group():
                continue
...
class ImageSaveStrategy(SaveStrategy):
    def __init__(self, format_name:str="PNG", background_color:str="white"):
        self.format_name:str = format_name#PNG, JPG, JPEG
