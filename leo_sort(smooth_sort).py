...
def smoothSort(arr):
    ...
    while True:
        maxI = None
        maxElem = None
        for i in range(len(heaps)):#выбираем максимальный корень из корней всех куч
            heap = heaps[i]
            el = heap[0] if (type(heap) == list) else heap#беру элемент в вершине кучи
            if maxElem == None or el > maxElem:
                maxI = i
                maxElem = el
        if maxI == None:#если при очередной итерации ничего не выбралось, то кучи кончились
            break
        maxHeap = heaps[maxI]##куча с макс вершиной
        ans.append(maxElem)#добавляем макс корень в ответ
        heaps.remove(heap[maxI])#нужно обновить рассмотренную кучку: удалить корень
        if type(maxHeap) == list:
            heaps.insert(maxI, maxHeap[2])
            heaps.insert(maxI, maxHeap[1])
    return ans[::-1]
arr = [3, 1, 2, 4, 22, 4, 2]
print(buildTree(arr))
